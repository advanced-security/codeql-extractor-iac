...
private import codeql.iac.ast.internal.TreeSitter
...
private module JoinBlockPredecessors {
  private predicate id(BICEP::AstNode x, BICEP::AstNode y) { x = y }

  private predicate idOf(BICEP::AstNode x, int y) = equivalenceRelation(id/2)(x, y)

  int getId(JoinBlockPredecessor jbp) {
    idOf(toBicepTreeSitter(jbp.getFirstNode().(AstCfgNode).getAstNode()), result)
    or
    idOf(toBicepTreeSitter(jbp.(EntryBasicBlock).getScope()), result)
  }

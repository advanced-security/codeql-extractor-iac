...
where (cd.getUser().toString() != "'root'" or cd.getPrivileged() = "true") 
    and td.getNetworkMode().toString() = "'host'"
select td, "ContainerDefinitions must not run as root or be privileged when networkmode Host is used"
